...
class TableUserOrder extends Component {
  state = {
    products: null,
    alert: null,
    type: "",
  ...
  render() {
    let columns = [
      { field: "number", headerName: "STT", flex: 0.6 },
      { field: "id", headerName: "Mã đơn hàng", flex: 1 },
      { field: "address", headerName: "Địa chỉ", flex: 1.7 },
      {
        field: "total",
    ...
    if (this.props.data)
      this.props.data
        .filter((i) => i.username === this.props.user.username)
        .forEach((item, index) => {
          let row = Object.assign({}, item);
          delete row._id;
          row.id = item._id;
          row.number = index + 1;
          row.state = "Đã đặt hàng";
...
export default connect((state) => ({ data: state.table.order }), {
  fetchOrder,
})(TableUserOrder);
